...
uses
  ...
  OptixCore.Protocol.Packet, NeoFlat.Panel;
// ---------------------------------------------------------------------------------------------------------------------
...
//    function GetFrameByCard(const ACard : TTabSheet) : TFrameRemoteShellInstance;
//    procedure TerminateShellInstance(const ACard : TCard);
...
//
//
//  if Pages.ActiveCardIndex <= -1 then
//    Exit();
//
//  result := Pages.ActiveCard.Tag = -1;
//end;
//
...
//begin
//  for var I := 0 to Pages.CardCount -1 do
//    TerminateShellInstance(Pages.Cards[I]);
//end;
//
//procedure TControlFormRemoteShell.TerminateShellInstance(const ACard : TCard);
//begin
//  if not Assigned(ACard) then
//    Exit();
//
//  var AFrame := GetFrameByTab(ACard);
...
//  ///
//
//  for var I := 0 to Pages.CardCount -1 do begin
//    var ACard := Pages.Cards[I];
//    ///
//
//    if (ACard.Controls[0] is TFrameRemoteShellInstance) and
//       (TFrameRemoteShellInstance(ACard.Controls[0]).InstanceId = AInstanceId) then begin
//
//       result := TFrameRemoteShellInstance(ACard.Controls[0]);
//
//       ///
//       break;
//    end;
...
//end;
//
//function TControlFormRemoteShell.GetFrameByCard(const ACard : TTabSheet) : TFrameRemoteShellInstance;
//begin
//  result := nil;
//  if not Assigned(ACard) then
//    Exit();
//
//  if (ACard.Controls[0] is TFrameRemoteShellInstance) then
//    result := TFrameRemoteShellInstance(ACard.Controls[0]);
//end;
//
...
//
//begin
//  var ACard := TCard.Create(Pages);
//
//  ACard. := Pages;
//  ACard.Caption     := GenerateRandomTabName();
//  ACard.Tag         := -1;
//
